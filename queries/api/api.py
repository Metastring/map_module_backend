...
from queries.service.service import fetch_polygon_query, fetch_multi_polygon_query, fetch_scientific_name_matches, fetch_multi_polygon_query_with_display_fields, fetch_scientific_name_matches_by_dataset
...
class Query:
    ...
    @strawberry.field(description="Search for spatial data by scientific name. Performs a case-insensitive partial match search across datasets (gbif, kew, cpmp) and returns all matching records with their associated geographic data. For cpmp, matches against the concatenated genus+species (and genus+species+author).")
