...
@router.post(
    "/register-shapefile",
    response_model=RegisterShapefileResponse,
    summary="Register Complete Shapefile",
    description="""
    Register a complete shapefile with a single API call. This endpoint:
    1. Uploads shapefile (ZIP) and publishes to GeoServer
    2. Configures GeoServer layer (SRS, bounding boxes, tile caching)
    3. Creates metadata entry
    4. Generates styles for multiple columns
    
    This is a comprehensive endpoint that orchestrates multiple operations for shapefiles.
    """,
)
async def register_shapefile(
    # File upload
    file: UploadFile = File(..., description="Shapefile ZIP file to upload"),
    # Form data as JSON string (will be parsed into RegisterShapefileFormData)
    form_data_json: str = Form(..., description="JSON string containing form data"),
    # Service dependency
    service: RegisterDatasetService = Depends(get_register_service),
):
    """
    Register a complete shapefile with upload, metadata, and styling.
    """
    try:
        # Parse form_data JSON string into RegisterShapefileFormData
        try:
            form_data_dict = json.loads(form_data_json)
            form_data = RegisterShapefileFormData(**form_data_dict)
        except json.JSONDecodeError as e:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid JSON in form_data: {str(e)}"
        ...
        file_extension = file.filename.lower()
        if not (file_extension.endswith(".zip") or file_extension.endswith(".shp")):
            raise HTTPException(
                status_code=400,
        ...
        request = RegisterShapefileRequest(
            ...
            contact_email=form_data.contact_email,
            style_configs=style_configs,
            data_source=DataSourceEnum.GEOSERVER,  # Shapefiles always use GeoServer data source
            publish_styles_to_geoserver=form_data.publish_styles_to_geoserver,
            attach_styles_to_layer=form_data.attach_styles_to_layer,
            user_id=form_data.user_id,
            user_email=form_data.user_email,
        ...
        # Call service
        result = await service.register_shapefile(request, file)
        return result

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error in register_shapefile endpoint: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
